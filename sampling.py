      ...
      #  view = views[rid]
        ...
        for picNO in range(1):
          with torch.no_grad():
            ...
            sparse_sinogram = cv2.imread('/media/lqg/Elements/HSK/XUEGUAN/extract64-512/1461.png',cv2.IMREAD_GRAYSCALE).astype(np.float32)
            ori_sinogram = cv2.imread('/media/lqg/Elements/HSK/XUEGUAN/png512-100BW_50/1461.png',cv2.IMREAD_GRAYSCALE).astype(np.float32)
            GT = np.copy(ori_sinogram)
            GT = GT[None,...]
            print("GT:",GT.shape)
            ...
            sparse_sinogram = sparse_sinogram.astype(np.float32)
            ori_sinogram = ori_sinogram.astype(np.float32)
            #将稀疏角图片转为小波
            sparse_sinogram = sparse_sinogram[None,...]
            sparse_sinogram = sparse_sinogram[None,...]#1,1,512,512
            ...
            #print(sparse_sinogram.shape)
            sparse_sinogram = dwt_data(sparse_sinogram)#4,256,256
            #print(sparse_sinogram.shape)
            ori_sinogram = ori_sinogram[None,...]
            ori_sinogram = torch.from_numpy(ori_sinogram[None,...]).cuda()#1,512,512
            sinogram_w = dwt_data(ori_sinogram)#4,256,256

            x0_A = sparse_sinogram[None, ...]#1,4,256,256
            #print(x0.shape)
            ...
            rec_img = np.zeros_like(GT)
            best_img = np.zeros_like(GT)
            r = 8
            ...
            for i in range(0,1000):
              ...
              if True:
                ...
                #rec_img = write_img(rec_img[0,...])

                current_img = (tmp - tmp.min())/(tmp.max() - tmp.min())
                cv2.imwrite("/media/lqg/Elements/HSK/SWORD-main/result_378_xueguan/1461/64_0_guocheng/"+str(i) +".png",current_img*255)
                rec_img = np.copy(tmp)
                #print(rec_img.shape)
                rec_img = rec_img[None,...]
                
                #print("chongjian:",rec_img.shape)
                #print("yuanshi:",GT.shape)
                psnr0,ssim0,mse0 = indicate(rec_img,GT)
                with open("/media/lqg/Elements/HSK/SWORD-main/result_378_xueguan/1461/64_0_guocheng/metrics.txt", "a") as f:
                     f.write(f"{i},{psnr0[0]},{ssim0[0]},{mse0[0]}\n")
              ...
              print("Step: {}   PSNR:{}  MSE:{}".format(i, np.round(psnr0[:4],3),np.round(1000*mse0[:4],3)))
